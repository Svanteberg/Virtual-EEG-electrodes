    ...
    def train(self):
        ...
        for epoch in range(0, self.epochs):
            ...
            for loop_index in range(0, len(self.train_subjects)):
                ...
                # update counter
                self.countVar.set(str(loop_index+1) + '/' + str(len(self.train_subjects)) + ' - ' + str(epoch + 1) + '/' + str(self.epochs) + ' - ' + str(epoch*len(self.train_subjects) + loop_index))
                ...
                if loop_index % 100 == 0:
                    ...
                    eeg_pred = self.generator.predict(eeg_test[:, self.input_channels, :, :])
...
#======================================test functions=========================================
    ...
    def training_test(self, num = 1000):
        ...
        for _ in np.arange(num):
            ...
            # mae fields
            mae_field = self.mae_field(self.reshape(eeg_test[:, :, self.output_channels], True)*norm_test, self.reshape(eeg_synthetic_test, True)*norm_test)
        ...
        return mae_train, mae_test, mae_fields
...
#=====================plot========================================
    ...
    def plot_eeg(self, frame ,eeg_pred, eeg_test):
        ...
        frame.ax.cla()
        spacing = 50
        t = np.arange(np.shape(eeg_test)[2])
        for j in range(5):
            norm = 50/np.max(np.abs(eeg_test[j]))
            for idx in range(self.output_size[1]):
                frame.ax.plot(t/self.sf + 1.1*j, norm*eeg_pred[j,idx] + spacing*self.output_channels[idx], color='black', linewidth=0.5)
            for idx in range(21):
                if [idx == i for i in self.input_channels]:
                    frame.ax.plot(t/self.sf + 1.1*j, norm*eeg_test[j,idx] + spacing*idx, color='tab:red', linewidth=0.5)
                else:
                    frame.ax.plot(t/self.sf + 1.1*j, norm*eeg_test[j,idx] + spacing*idx, color='tab:blue', linewidth=0.5)
        # auto encoder
        #
        #frame.ax.set_xticks(np.arange(0,5,)) #np.round(len(t)/self.sf)+1,1))
    ...
    def plot_progression(self):
        ...
        mae_train,mae_test,test_field = self.training_test()
        self.mae_train.append(mae_train)
        self.mae_test.append(mae_test)
        self.mae_fields.append(test_field)
        ...
        self.trainingPlot.ax.plot(np.arange(len(self.mae_test)), self.mae_test, color = 'tab:orange', label = 'MAE validation')
    ...
    def plot_field(self,mat):
        ...
        for i in range(self.output_size[1]):
            ...
            self.field_plot.ax.text(self.col_map[i], self.row_map[i],string,
                                    horizontalalignment = "center", color = "white" if mat[self.row_map[i], self.col_map[i]] < thresh else "black")
...
#=====================misc========================================
    ...
    def print_eta(self,start_time,epoch,loop_index):
        ...
        eta = round((passed_time/(epoch*len(self.train_subjects) + loop_index))*(self.epochs*len(self.train_subjects) - (epoch*len(self.train_subjects) + loop_index)))
...
class labelWithOption(object):
    ...
    def __init__(self, frame, text, nums, init_var, row, column, width, color):
        ...
        Label(frame, text = text, background = color).grid(r ow =r ow, column = column, padx = 5, pady = 5)
...
topFrame_L.pack(side = LEFT,f ill = BOTH, padx = 5, pady = 5)
topFrame_R = Frame(topFrame, background =c olor)
...
G.iterations_input = labelWithOption(bottomFrame, 'Iterations', [100, 500, 1000, 10000, 100000, 1000000, 5000000], 5000000, 1, 1, W, color)
