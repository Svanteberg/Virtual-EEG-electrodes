...
class eeg_plotter():
    ...
    def open_subject(self):
        ...
        self.index = 0
        self.eeg = np.load(self.directory,allow_pickle=True)
        self.plot_eeg()
    ...
    def plot_eeg(self):
        ...
        for idx in range(21):
            if idx == 2 or idx == 6 or idx == 8 or idx == 10 or idx == 12 or idx == 14 or idx == 19:
                self.eeg_plot.ax.plot(t/self.sampling_frequency,self.amp*self.eeg[self.index][1][idx]+spacing*idx,color='tab:blue',linewidth=0.5)
            else:
                self.eeg_plot.ax.plot(t/self.sampling_frequency,self.amp*self.eeg[self.index][1][idx]+spacing*idx,color='tab:red',linewidth=0.5)
        for idx in range(7):
            self.eeg_plot.ax.plot(t/self.sampling_frequency,self.amp*self.eeg[self.index][0][idx]+spacing*self.out_ch[idx],color='black',linewidth=0.5)
