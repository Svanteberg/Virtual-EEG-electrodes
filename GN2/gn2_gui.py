...
class generator_2():
    def __init__(self):
        ...
        self.row_map = [2, 2, 1, 3, 1, 3, 2]
        self.col_map = [1, 5, 2, 2, 4, 4, 3]
        # network parameters
        self.layers = 4
        self.strides = 2
        # training parameters
            ...
            for loop_index in range(0, len(self.train_subjects)):
                ...
                if loop_index % 100 == 0:
                    ...
                    if self.use_data_normalization:
                        ...
                        eeg_pred = np.reshape(eeg_pred, (7, 2560))*norm
                    else:
                        eeg_test = np.reshape(eeg_test, (21, 2560))
                        eeg_pred = np.reshape(eeg_pred, (7, 2560))
...
#======================================test functions=========================================
    ...
    def training_test(self, num = 1000):
        ...
        for _ in np.arange(num):
            ...
            mae_val.append(self.mae(np.reshape(eeg_test[:, self.output_electrodes, :, :], (7, 2560))*norm_test ,np.reshape(eeg_synthetic_test, (7, 2560))*norm_test))
            # mae fields
            mae_field = self.mae_field(np.reshape(eeg_test[:, self.output_electrodes, :, :], (7, 2560))*norm_test, np.reshape(eeg_synthetic_test, (7, 2560))*norm_test)
