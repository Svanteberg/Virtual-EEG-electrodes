...
There are several options when using the code that is provided. A simpler step-by-step demo for training a GN1 type network is given here: [demo](https://github.com/Svanteberg/Virtual-EEG-electrodes/blob/master/step-by-step_demo.md). Further down below, more in-depth information and general instructions are given.
...
The provided small sample (10 subjects, each with 5 - 15 x 10 s of data) of artificial data was generated using this script.
...
The time consumtion for training depend on data size and hardware. An epoch of training is here defined as training one time with one example from each subject. This mean that the time for training an epoch increases with the number of subjects. Using the artificial data generated by running the `python3.7 art_eeg.py` script, the default number of epochs (100) of `python3.7 gn1.py` training on an Nvidia Titan X consumed 2.5 - 3 hours. The artificial data is relatively well-behaved, so less than 100 epochs may be adequate, but real data may instead need more training time. In hour project we used 1,114 subjects and trained for 1,000 epochs which usually consumed 15 - 30 hours (where the variation was due to difference in hardware but also some variation in hyperparameters).
