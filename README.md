...
The time consumtion for training depend on data size and hardware. An epoch of training is here defined as training one time with one example from each subject. This mean that the time for training an epoch increases with the number of subjects. Using the artificial data generated by running the `python3.7 art_eeg.py` script, the default number of epochs (100) of `python3.7 art_eeg.py` training on an Nvidia Titan X consumed 2.5 - 3 hours. The artificial data is relatively well-behaved, so real data may need more training time. In hour project we used 1,114 subjects, trained for 1,000 epochs, and consumed 15 - 30 hours (where the variation was due to difference in hardware but also some variation in hyperparameters).
