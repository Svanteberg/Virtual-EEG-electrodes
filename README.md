...
The time consumtion for training depend on data size and hardware. An epoch of training is here defined as training one time with one example from each subject. This mean that the time for training an epoch increases with the number of subjects. In a test run using the artificial data (generated by running the `python3.7 art_eeg.py` script), the default number of epochs (1000) of `python3.7 gn1.py` consumed 28 hours  when training on an Nvidia Titan X.
