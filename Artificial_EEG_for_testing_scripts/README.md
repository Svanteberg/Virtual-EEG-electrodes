To facilitate the process of setting up an organization for the data, an example is provided containing artificial EEG data. The set contain EEG from 10 subjects with 1 - 3 recordings per subject, each recording consisting of 5 x 10 s of data. The subject id:s are in the file `subjects_folders` and the index list is in `indices.npy`. This small dataset is only for illustration purpose only — more data will have to be generated to train networks.
